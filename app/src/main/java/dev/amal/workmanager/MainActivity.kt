...
import dev.amal.workmanager.ui.theme.WorkManagerTheme
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            WorkManagerTheme {
                val worksInfos = workManager
                    .getWorkInfosForUniqueWorkLiveData("download")
                    .observeAsState()
                    .value

                val downloadInfo = remember(key1 = worksInfos) {
                    worksInfos?.find { it.id == downloadRequest.id }
                }
                val filterInfo = remember(key1 = worksInfos) {
                    worksInfos?.find { it.id == colorFilterRequest.id }
                }
